...
public abstract class DominantChecker {
  ...
  MatchEnum checkFamily(VcfRecord vcfRecord, Pedigree family) {
    ...
    Set<Sample> samplesMissingGt = membersByStatus.get(AffectedStatus.MISSING);
    if (samplesMissingGt != null && !samplesMissingGt.isEmpty()) {
  ...
  static void checkAffectedGenotypes(
      Set<Genotype> affectedGenotypes, Set<MatchEnum> matches, Genotype genotype) {
    for (Genotype affectedGenotype : affectedGenotypes) {
      if (affectedGenotype.hasAltAllele()
          && affectedGenotype.getAlleles().stream()
              .filter(allele -> allele.isCalled() && allele.isNonReference())
              .allMatch(allele -> genotype.getAlleles().contains(allele))) {
        matches.add(FALSE);
      } else {
        matches.add(POTENTIAL);
      }
  ...
  protected abstract MatchEnum checkUnaffected(
      VcfRecord vcfRecord,
      Map<AffectedStatus, Set<Sample>> membersByStatus,
      Set<Genotype> affectedGenotypes);

  protected abstract MatchEnum checkAffected(
      VcfRecord vcfRecord,
      Map<AffectedStatus, Set<Sample>> membersByStatus,
      Set<Genotype> affectedGenotypes);
}
