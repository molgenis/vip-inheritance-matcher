...
public class DeNovoChecker {
  ...
  // use original GT instead of inheritance matcher specific one since we are looking for "new
  // alleles" not specifically pathogenic ones
  public MatchEnum checkDeNovo(VcfRecord vcfRecord, Sample proband) {
    ...
    if ((contigId != null && ContigUtils.isChromosomeX(contigId))
        && proband.getPerson().getSex() == Sex.MALE) {
  ...
  private static MatchEnum checkMaleXVariant(
      @Nullable Genotype probandGt, @Nullable Genotype motherGt) {
  ...
  private static MatchEnum checkYLinkedVariant(
      Sample proband, @Nullable Genotype probandGt, @Nullable Genotype fatherGt) {
    return switch (proband.getPerson().getSex()) {
      case MALE -> checkYLinkedVariantMale(probandGt, fatherGt);
      case FEMALE -> FALSE;
      default ->
          ((fatherGt != null && fatherGt.hasAltAllele())
                  || (probandGt != null && !probandGt.hasAltAllele()))
              ? FALSE
              : POTENTIAL;
    };
  }

  private static MatchEnum checkYLinkedVariantMale(
      @Nullable Genotype probandGt, @Nullable Genotype fatherGt) {
    if (probandGt != null && probandGt.hasAltAllele()) {
      if (fatherGt != null && hasSameAltAlleles(probandGt, fatherGt)) {
        return FALSE;
      } else {
        return (fatherGt != null
                && !hasMissing(fatherGt)
                && !hasSameAltAlleles(probandGt, fatherGt))
            ? TRUE
            : POTENTIAL;
      }
    } else if (hasMissing(probandGt)) {
      return fatherGt != null && fatherGt.hasAltAllele() ? FALSE : POTENTIAL;
    }
    return FALSE;
  }

  private static boolean hasSameAltAlleles(Genotype probandGt, Genotype fatherGt) {
    return probandGt.getAlleles().stream()
        .filter(allele -> allele.isCalled() && allele.isNonReference())
        .allMatch(allele -> fatherGt.getAlleles().contains(allele));
  }

  private static MatchEnum checkMtVariant(
      @Nullable Genotype probandGt, @Nullable Genotype motherGt) {
    if (probandGt == null || probandGt.isNoCall()) {
      return POTENTIAL;
    } else if (probandGt.hasAltAllele()) {
      if (motherGt != null && hasSameAltAlleles(probandGt, motherGt) && !motherGt.hasReference()) {
        return FALSE;
      } else if (motherGt != null
          && !hasSameAltAlleles(probandGt, motherGt)
          && motherGt.isCalled()
          && !motherGt.isMixedOrHet()) {
        return TRUE;
      } else {
        return POTENTIAL;
      }
    } else if (probandGt.hasMissingAllele()) {
      if (probandGt.hasAltAllele() && motherGt != null && hasSameAltAlleles(probandGt, motherGt)) {
        return FALSE;
      } else {
        return POTENTIAL;
      }
  ...
  private MatchEnum checkRegular(
      @Nullable Genotype probandGt, @Nullable Genotype fatherGt, @Nullable Genotype motherGt) {
    ...
    if (alleles.size() != 2) {
      throw new UnsupportedOperationException(
          "Checking 'regular' denovo calles requires diploid genotypes.");
    }
  ...
  private static MatchEnum checkAlleles(
      @Nullable Genotype fatherGt, @Nullable Genotype motherGt, Allele allele1, Allele allele2) {
  ...
  private static MatchEnum checkPartialCalls(
      @Nullable Genotype fatherGt, @Nullable Genotype motherGt, Allele allele1, Allele allele2) {
  ...
  private static boolean containsAlleleOrRef(@Nullable Genotype genotype, Allele allele) {
    return allele.isReference() || (genotype != null && genotype.getAlleles().contains(allele));
  }

  private static boolean containsAlleleOrNoCall(@Nullable Genotype genotype, Allele allele1) {
