...
public abstract class InheritanceChecker {
  ...
  protected MatchEnum checkFamily(VcfRecord vcfRecord, Pedigree family) {
    ...
    affectedGenotypes.forEach(
        genotype ->
            genotype.getAlleles().stream()
                .filter(allele -> allele.isNonReference() && allele.isCalled())
                .forEach(affectedAltAlleles::add));
    matches.add(checkUnaffected(vcfRecord, membersByStatus, affectedAltAlleles));
    if (membersByStatus.get(AffectedStatus.MISSING) != null
        && !membersByStatus.get(AffectedStatus.MISSING).isEmpty()) {
