...
public class InheritanceService {
  ...
  public InheritanceService(
      Annotator annotator,
      PedigreeInheritanceChecker pedigreeInheritanceChecker,
  ...
  private void matchVcfRecord(
      RecordWriter recordWriter,
      VcfRecord vcfRecord,
      Collection<Pedigree> pedigrees,
      Map<GeneInfo, Set<VcfRecord>> vcfRecordGeneInfoMap) {
    ...
    VariantContext annotatedVc =
        annotator.annotateInheritance(vcfRecord, pedigrees, inheritanceResultMap, probands);
    recordWriter.add(annotatedVc);
  }

  private void matchPedigree(
      VcfRecord vcfRecord,
      Collection<Pedigree> pedigrees,
      Map<GeneInfo, Set<VcfRecord>> vcfRecordGeneInfoMap,
    ...
    for (Pedigree pedigree : pedigrees) {
      ...
      if (altAllelesForPedigree.stream()
          .anyMatch(allele -> vcfRecord.pathogenicAlleles().contains(allele))) {
        addToVcfRecordMap(vcfRecord, vcfRecordGeneInfoMap);
        ...
        modes.forEach(
            mode -> {
              ...
              if (isMatch != FALSE) {
                inheritanceResult.addInheritanceMode(
                    new PedigreeInheritanceMatch(mode, isMatch == POTENTIAL));
              }
            });
        if (inheritanceResult.getPedigreeInheritanceMatches().stream()
            .noneMatch(match -> match.inheritanceMode() == AD)) {
          MatchEnum isAdIpMatch = pedigreeInheritanceChecker.check(vcfRecord, pedigree, AD_IP);
          if (isAdIpMatch != FALSE) {
            inheritanceResult.addInheritanceMode(
                new PedigreeInheritanceMatch(AD_IP, isAdIpMatch == POTENTIAL));
          }
        }
  ...
  private void matchCompounds(
    ...
    Map<GeneInfo, Set<CompoundCheckResult>> compoundsMap =
        arCompoundChecker.check(vcfRecordGeneInfoMap, vcfRecord, pedigree);
  ...
  private void addToVcfRecordMap(
      VcfRecord vcfRecord, Map<GeneInfo, Set<VcfRecord>> vcfRecordGeneInfoMap) {
    for (GeneInfo geneInfo : vcfRecord.geneInfos()) {
      Set<VcfRecord> records =
          vcfRecordGeneInfoMap.containsKey(geneInfo)
              ? vcfRecordGeneInfoMap.get(geneInfo)
              : new HashSet<>();
  ...
  private Set<Pedigree> createFamilyFromVcf(VCFHeader fileHeader) {
    ...
    for (String sampleName : sampleNames) {
      // no ped: unknown Sex, assume affected, no relatives, therefor the sampleId can be used as
      // familyId
      Person person =
          Person.builder()
              .familyId(sampleName)
              .individualId(sampleName)
              .paternalId("")
              .maternalId("")
              .sex(Sex.UNKNOWN)
      ...
      familyList.add(
          Pedigree.builder().id(sampleName).members(singletonMap(sampleName, sample)).build());
    }
    return familyList;
  }
}
