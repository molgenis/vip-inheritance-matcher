...
public class VcfRecordFactoryImpl implements VcfRecordFactory {
  ...
  public VcfRecord create(VariantContext variantContext, Set<String> pathogenicClasses) {
    ...
    Set<Allele> pathogenicAlleles =
        getPathogenicAlleles(variantContext, vepMetadata, pathogenicClasses);
  ...
  private Set<GeneInfo> getVcfGeneInfos(VariantContext variantContext) {
    ...
    if (vepMetadata.getVepFieldId() != null) {
      List<String> vepValues =
          getAttributeAsStringList(variantContext, vepMetadata.getVepFieldId());
      for (String vepValue : vepValues) {
        ...
        Set<InheritanceMode> inheritanceModes =
            vepMetadata.getInheritanceIndex() != -1
                ? mapGeneInheritance(vepSplit[vepMetadata.getInheritanceIndex()])
                : emptySet();
        if (!geneId.isEmpty()) {
  ...
  private Set<Allele> getPathogenicAlleles(
      VariantContext variantContext, VepMetadata vepMetadata, Set<String> pathogenicClasses) {
    ...
    for (int i = 1; i <= variantContext.getAlternateAlleles().size(); i++) {
      ...
      if (pathogenicClasses.isEmpty()
          || isAllelePathogenic(variantContext, vepMetadata, i, pathogenicClasses)) {
  ...
  public boolean isAllelePathogenic(
    ...
    if ((vepMetadata.getAlleleNumIndex() == -1 || vepMetadata.getClassIndex() == -1)) {
      throw new UnsupportedOperationException(
          "Classes mapping provided, but input VCF is missing ALLELE_NUM or VIPC VEP annotations");
    }
    if (vepMetadata.getVepFieldId() == null) {
      throw new UnsupportedOperationException(
          "Cannot determine pathogenic alleles without VEP annotations.");
    }
