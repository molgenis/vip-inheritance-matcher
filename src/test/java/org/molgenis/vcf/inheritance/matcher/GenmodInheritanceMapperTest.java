...
class GenmodInheritanceMapperTest {
  ...
  void mapInheritance() {
    ...
    Annotation ann1 = Annotation.builder().familyID("FAM001").inheritanceMode(
    ...
    Annotation ann2 = Annotation.builder().familyID("FAM002").inheritanceMode(
        Set.of(InheritanceMode.builder().inheritanceModeEnum(AD).build(),
            InheritanceMode.builder().inheritanceModeEnum(AR).subInheritanceMode(
                SubInheritanceMode.HOM).build())).denovo(false).build();
    Annotation ann3 = Annotation.builder().familyID("FAM003").inheritanceMode(
        Set.of(InheritanceMode.builder().inheritanceModeEnum(AD).build(),
            InheritanceMode.builder().inheritanceModeEnum(AR).subInheritanceMode(
                SubInheritanceMode.COMP).build())).denovo(false).build();
    List<Annotation> expected = Arrays.asList(ann1, ann2, ann3);
  ...
  void mapInheritanceX() {
    ...
    Annotation ann3 = Annotation.builder().familyID("FAM003")
        .inheritanceMode(Set.of(InheritanceMode.builder().inheritanceModeEnum(XD).build()))
        .denovo(false).build();
    List<Annotation> expected = Arrays.asList(ann1, ann2, ann3);
    assertEquals(expected, mapper.mapInheritance(vc));
  }
