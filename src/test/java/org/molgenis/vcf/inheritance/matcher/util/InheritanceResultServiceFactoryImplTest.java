...
class InheritanceResultServiceFactoryImplTest {
  @Mock private Annotator annotator;
  @Mock private PedigreeInheritanceChecker pedigreeInheritanceChecker;

  private InheritanceServiceFactoryImpl inheritanceServiceFactoryImpl;

  @BeforeEach
  void setUp() {
    inheritanceServiceFactoryImpl =
        new InheritanceServiceFactoryImpl(annotator, pedigreeInheritanceChecker);
  }

  @Test
  void create() {
    InheritanceService inheritanceServiceFactory =
        inheritanceServiceFactoryImpl.create(Settings.builder().build());
