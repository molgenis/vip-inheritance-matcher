...
class ArCheckerTest {
  ...
  void check(VcfRecord vcfRecord, Pedigree family, String expectedString, String displayName) {
    MatchEnum expected = mapExpectedString(expectedString);
    assertEquals(expected, arChecker.check(vcfRecord, family));
  }

  private static Stream<Arguments> provideTestCases() throws IOException {
    File testFile = ResourceUtils.getFile("classpath:ARTests.tsv");
    ...
    return lines.stream()
        ...
        .map(
            line -> {
              ...
              return Arguments.of(
                  VariantContextTestUtil.createVariantContext(genotypes, ""),
                  family,
                  expected,
                  testName);
            });
